...
const actualizarMensaje = () => {
  ...
  //Tomo el boton para agregar
    const boton = document.querySelector("button");
  // Guardar el b dentro del h3
  const espaciosLibres = document.querySelector("h3");
  ...
  if (celdasVacias.length === 0) {
      ...
      espaciosLibres.classList.add("rojo");
      boton.disabled = true;
      input.disabled = true;
  }else if (celdasVacias.length > 0) {
    console.log(celdasVacias.length);
    espaciosLibres.innerText = `Tienes ${celdasVacias.length} espacios libres`;
    espaciosLibres.classList.remove("rojo");
